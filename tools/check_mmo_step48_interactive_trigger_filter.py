#!/usr/bin/env python3
"""Check Step48 interactive/trigger/mover live-state coverage.

Step48 tightens Step47 interactive capture so bootstrap/TA mobsi state materialization is not mistaken for gameplay. It also adds capture-only trigger/mover evidence for hidden doors, gates and mover chains caused by a player interaction.
"""
...
DOMAIN_KINDS: dict[str, set[str]] = {
    ...
    "interactive": {"use_interactive", "update_interactive_state"},
    "trigger_mover": {"trigger_event", "mover_state_changed"},
...
EXPECTED_DB_EVENT_BY_KIND = {
    ...
    "update_interactive_state": "interactive_state_changed",
}

CAPTURE_ONLY_KINDS = {"drop_character_item", "loot_npc_inventory", "ready_weapon", "holster_weapon", "use_interactive", "trigger_event", "mover_state_changed", "character_resource_delta", "world_time_changed"}
...
def interactive_noise_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    bad_samples: list[dict[str, Any]] = []
    total_update = 0
    causal_update = 0
    direct_player = 0
    for row in rows:
        if action_kind(row) != "update_interactive_state":
            continue
        total_update += 1
        payload = payload_of(row)
        cause = str(payload.get("capture_cause") or "")
        actor_key = payload.get("actor_key")
        player_caused = payload.get("player_caused") is True or str(payload.get("player_caused")).lower() == "true"
        if cause in {"direct_player_actor", "recent_player_interactive_use", "recent_player_world_interaction"} or player_caused:
            causal_update += 1
        elif is_player_key(actor_key):
            direct_player += 1
            causal_update += 1
        elif len(bad_samples) < 20:
            bad_samples.append({
                "target_key": row.get("target_key") or payload.get("target_key") or payload.get("interactive_key"),
    ...
    return {
        "total_update_interactive_state": total_update,
        "causal_update_interactive_state": causal_update,
        "direct_player_update_interactive_state": direct_player,
        "uncausal_update_interactive_state": max(0, total_update - causal_update),
        "uncausal_samples": bad_samples,
    }
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Check Step48 live interactive/trigger/mover gameplay coverage from server JSONL/outbox artifacts.")
    ...
    ap.add_argument("--max-uncausal-interactive-state", type=int, default=0, help="maximum update_interactive_state rows without a player-caused capture_cause")
    ...
    interactive_noise = interactive_noise_summary(accepted)
    ...
    if args.fail_on_mysql_errors and mysql_errors:
        status = "failed"
    if interactive_noise["uncausal_update_interactive_state"] > args.max_uncausal_interactive_state:
        status = "failed"
    ...
    manifest = {
        ...
        "interactive_noise": interactive_noise,
    ...
    print(f"OK: Step48 domains={ {k:v['total'] for k,v in domains.items() if v['present']} }")
    if interactive_noise["uncausal_update_interactive_state"]:
        print("Interactive state noise:")
        print("  " + json.dumps(interactive_noise, ensure_ascii=False, sort_keys=True))
    if world_ai:
