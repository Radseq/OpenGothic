#!/usr/bin/env python3
"""Check Step206 server content import jobs DB objects and health."""
...
def object_exists(target: Target, kind: str, name: str) -> int:
    ...
    if kind == "procedure":
        return scalar_int(target, "SELECT COUNT(*) FROM information_schema.routines WHERE routine_schema=DATABASE() AND routine_type='PROCEDURE' AND routine_name=" + sql_literal(name) + ";")
    raise ValueError(kind)


def main() -> int:
    parser = argparse.ArgumentParser(description="Check Step206 server content import job DB state.")
    parser.add_argument("--url", required=True, help="mysql://user:password@host:port/database")
    parser.add_argument("--expect-jobs", action="store_true")
    parser.add_argument("--fail-on-failed-jobs", action="store_true")
    ...
    for kind, name in (
        ("table", "mmo_server_content_import_jobs"),
        ("view", "v_mmo_server_content_import_jobs"),
        ("view", "v_mmo_server_content_import_job_health"),
        ("procedure", "mmo_enqueue_server_content_import_job"),
    ):
        count = object_exists(target, kind, name)
        checks.append({"name": name, "kind": kind, "status": "passed" if count == 1 else "failed", "count": count})

    migration_count = scalar_int(target, "SELECT COUNT(*) FROM mmo_schema_versions WHERE migration_key='server/sql/step206_server_content_import_jobs.sql';")
    checks.append({"name": "step206_schema_version_marker", "kind": "migration", "status": "passed" if migration_count == 1 else "failed", "count": migration_count})

    job_count = scalar_int(target, "SELECT COUNT(*) FROM mmo_server_content_import_jobs;")
    checks.append({"name": "content_import_job_rows", "kind": "data", "status": "passed" if job_count > 0 else "warning", "count": job_count})
    ...
    checks.append({"name": "content_import_job_health", "kind": "data", "status": "passed", "details": health})

    if args.expect_jobs:
        checks.append({"name": "expected_content_import_jobs", "kind": "data_expectation", "status": "passed" if job_count > 0 else "failed", "count": job_count})
    if args.fail_on_failed_jobs:
        failed_jobs = scalar_int(target, "SELECT COUNT(*) FROM mmo_server_content_import_jobs WHERE job_status='failed';")
        checks.append({"name": "failed_content_import_jobs", "kind": "data_expectation", "status": "passed" if failed_jobs == 0 else "failed", "count": failed_jobs})

    failed = [item for item in checks if item["status"] == "failed"]
    report = {"tool": "check_mmo_step206_server_content_import_jobs.py", "status": "failed" if failed else "passed", "checked_at": datetime.now(timezone.utc).isoformat(), "checks": checks}
    if args.output:
