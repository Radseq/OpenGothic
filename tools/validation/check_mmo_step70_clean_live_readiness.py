...
def run_mysql(target: Target, sql: str) -> str:
    proc = subprocess.run(mysql_cmd(target), input=sql, text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
...
def command_plan(mysql_url: str, session_key: str) -> dict[str, list[str]]:
    ...
    return {
        "receiver": [
            "python3 tools/production/run_mmo_pc_hero_test_receiver.py",
            f'  --url "{mysql_url}"',
        ],
        "game": [
        ...
        "worker": [
            "python3 tools/production/run_mmo_live_resolved_worker.py",
            f'  --url "{mysql_url}"',
            "  --max-actions 500",
            "  --reset-matching-failed",
        ],
        "roundtrip_check_after_actions": [
            "python3 tools/validation/check_mmo_step69_pc_hero_test_inventory_roundtrip.py",
            f'  --url "{mysql_url}"',
