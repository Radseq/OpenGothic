#!/usr/bin/env python3
"""Check Step47 interactive/mobsi live-state coverage.

Step47 adds the missing Gothic mobsi/interactive path: using fireplaces, levers, doors, switches and other interactives now produces server-boundary evidence.
The checker keeps Step46 gameplay coverage and adds an interactive domain for use_interactive/update_interactive_state.
"""
...
CAPTURE_ONLY_KINDS = {"drop_character_item", "loot_npc_inventory", "ready_weapon", "holster_weapon", "use_interactive", "character_resource_delta", "world_time_changed"}
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Check Step47 live interactive/mobsi gameplay coverage from server JSONL/outbox artifacts.")
    ...
    print(f"OK: Step47 domains={ {k:v['total'] for k,v in domains.items() if v['present']} }")
    if world_ai:
