#!/usr/bin/env python3
"""Run the Step46 post-capture MySQL follow-up pipeline.
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Step46 worker follow-up: optional pickup fixture, worker, consumable/sleep/world-AI domain checker.")
    ap.add_argument("--url", required=True)
    ap.add_argument("--session-key", required=True)
    ap.add_argument("--worker-id", default="dev-resolved-worker-step46")
    ap.add_argument("--max-actions", type=int, default=1000)
    ap.add_argument("--accepted-jsonl", default="runtime/mmo_server_actions_step46.jsonl")
    ap.add_argument("--checkpoint-jsonl", default="runtime/mmo_server_checkpoints_step46.jsonl")
    ap.add_argument("--rejected-jsonl", default="runtime/mmo_server_rejects_step46.jsonl")
    ap.add_argument("--summary-json", default="runtime/mmo_server_step46_summary.json")
    ap.add_argument("--output", default="runtime/mmo_step46_consumables_sleep_ai_context_check.json")
    ap.add_argument("--manifest", default="runtime/mmo_step46_consumables_sleep_followup_manifest.json")
    ...
    checker = script_dir / "check_mmo_step46_consumables_sleep_ai_context.py"
    check_cmd = [
    ...
    results.append(run("step46_domain_check", check_cmd, allow_failure=True))
