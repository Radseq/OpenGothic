#!/usr/bin/env python3
"""Run the Step48 interactive/trigger/mover post-capture follow-up pipeline.

The live server has already captured/enqueued actions. This wrapper reruns the resolved worker and builds a Step48 report focused on player-caused interactive state plus trigger/mover evidence.
"""
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Step48 worker follow-up: optional pickup fixture, worker, interactive/trigger/mover domain checker.")
    ap.add_argument("--url", required=True)
    ap.add_argument("--session-key", required=True)
    ap.add_argument("--worker-id", default="dev-resolved-worker-step48")
    ap.add_argument("--max-actions", type=int, default=1000)
    ap.add_argument("--accepted-jsonl", default="runtime/mmo_server_actions_step48.jsonl")
    ap.add_argument("--checkpoint-jsonl", default="runtime/mmo_server_checkpoints_step48.jsonl")
    ap.add_argument("--rejected-jsonl", default="runtime/mmo_server_rejects_step48.jsonl")
    ap.add_argument("--summary-json", default="runtime/mmo_server_step48_summary.json")
    ap.add_argument("--output", default="runtime/mmo_step48_interactive_trigger_filter_check.json")
    ap.add_argument("--manifest", default="runtime/mmo_step48_interactive_trigger_followup_manifest.json")
    ...
    ap.add_argument("--max-uncausal-interactive-state", type=int, default=0)
    ...
    checker = script_dir / "check_mmo_step48_interactive_trigger_filter.py"
    check_cmd = [
    ...
    check_cmd.extend(["--max-uncausal-interactive-state", str(args.max_uncausal_interactive_state)])
    for domain in args.require_domain:
        check_cmd.extend(["--require-domain", domain])
    results.append(run("step48_domain_check", check_cmd, allow_failure=True))
