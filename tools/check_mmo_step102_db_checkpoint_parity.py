"""Audit Step102 DB save checkpoint parity against live projections."""
...
def strict_restore_row(target: Target, session_key: str, character_key: str) -> dict[str, object]:
    ...
    out = dict(zip(cols, rows[-1], strict=False))
    for key in cols[4:15] + ("strict_restore_ok",):
        out[key] = int_value(out.get(key))
    return out
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Audit DB save checkpoint snapshot parity against live projections.")
    ...
    ap.add_argument("--assert-no-drift", action="store_true", help="Return non-zero unless snapshot counts equal live projection counts.")
    ap.add_argument("--output", default="")
    args = ap.parse_args()

    target = parse_mysql_url(args.url)
    result: dict[str, object] = {
        "step": "102_db_checkpoint_parity",
    ...
    try:
        ...
        if missing:
            ...
            if not manifest.get("manifest_hex"):
                ...
                no_drift = all(item.get("equal") is True for item in result["count_drift"].values())  # type: ignore[union-attr]
