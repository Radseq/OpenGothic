#!/usr/bin/env python3
"""Check Step44 live gameplay domain coverage.

This checker is intentionally broader than the Step43 movement checker. It
...
particular, drop_character_item is capture-only in Step44 until a canonical
...
DOMAIN_KINDS: dict[str, set[str]] = {
    ...
    "resource": {"consume_mana", "consume_item"},
...
CAPTURE_ONLY_KINDS = {"drop_character_item"}
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Check Step44 live gameplay domain coverage from server JSONL/outbox artifacts.")
    ...
    print(f"OK: Step44 domains={ {k:v['total'] for k,v in domains.items() if v['present']} }")
    if missing_domains:
