...
def main() -> int:
    ...
    materialize_cmd = [
        sys.executable,
        str(ROOT / "tools" / "migration" / "materialize_mmo_server_read_model_v1.py"),
        "--url",
        args.url,
        "--output",
    ...
    inspect_cmd = [
        sys.executable,
        str(ROOT / "tools" / "migration" / "inspect_mmo_server_read_model_v1.py"),
        "--url",
        args.url,
        "--output",
        str(inspect_report),
    ...
    if rc_materialize == 0:
        rc_export = run([
            sys.executable,
            str(ROOT / "tools" / "migration" / "export_mmo_server_materialization_manifest.py"),
            "--url",
            args.url,
            "--output",
