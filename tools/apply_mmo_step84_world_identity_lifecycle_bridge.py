#!/usr/bin/env python3
"""Install Step84 world identity/lifecycle bridge procedures."""
...
SQL_PATH = ROOT / "server" / "sql" / "step84_world_identity_lifecycle_bridge.sql"
REQUIRED_ROUTINES = ("mmo_grant_character_item_by_symbol",)
...
def inspect(target: Target) -> dict[str, object]:
    return {"routines": {name: count_routine(target, name) == 1 for name in REQUIRED_ROUTINES}}


def main() -> int:
    ap = argparse.ArgumentParser(description="Install Step84 world identity/lifecycle bridge procedures.")
    ...
    result: dict[str, object] = {
        "step": "84_world_identity_lifecycle_bridge",
    ...
    try:
        if not sql_path.exists():
            ...
            missing = [name for name, ok in result["routines"].items() if not ok]  # type: ignore[index]
            if missing:
                result["status"] = "failed"
                result["missing_routines"] = missing
            else:
