#!/usr/bin/env python3
"""Apply the standalone MMO content-build database surface.

This intentionally does not apply runtime MMO migrations. It creates/updates the
separate mmo_content_build schema used by future server-side ZEN/DAT/OU parsers.
"""
...
CONTENT_BUILD_SQL_SURFACES = (
    SqlSurface(
        "content_build_database",
        ROOT / "server" / "sql" / "step211_content_build_database.sql",
        "standalone mmo_content_build schema for server-side ZEN/DAT/OU parser output",
    ),
    SqlSurface(
        "content_build_item_templates_compat",
        ROOT / "server" / "sql" / "step221_content_build_item_templates_compat.sql",
        "compatibility surface for DAT item template candidates and item_template_count health",
    ),
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply standalone MMO content-build database SQL.")
    parser.add_argument("--url", default="", help="mysql://user:password@host:port/existing_database")
    parser.add_argument("--output", default="")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--list", action="store_true", help="Print the content-build SQL surface order and exit.")
    args = parser.parse_args()

    if args.list:
        for surface in CONTENT_BUILD_SQL_SURFACES:
    ...
    manifest: dict[str, object] = {
        "tool": "apply_content_build_database.py",
        "status": "running",
    ...
    missing = [surface for surface in CONTENT_BUILD_SQL_SURFACES if not surface.path.exists()]
    if missing:
