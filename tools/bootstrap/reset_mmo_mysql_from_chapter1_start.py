...
def main() -> int:
    ...
    if reset_result["returncode"] != 0:
        ...
        for migration in base_migrations():
            ...
            import_cmd = [
                sys.executable,
                str(ROOT / "tools" / "bootstrap" / "import_runtime_sqlite_to_mysql.py"),
                "--sqlite", str(baseline),
            ...
            if import_result["returncode"] != 0:
                ...
                if manifest["status"] == "running":
                    check_cmd = [
                        sys.executable,
                        str(ROOT / "tools" / "validation" / "check_mysql_bootstrap_import.py"),
                        "--url", mysql_url_for_database(args.mysql_url, target.database),
                        "--realm-key", args.realm_key,
                        "--character-key", args.character_key,
                    ]
                ...
                if manifest["status"] == "passed" and args.with_step55_live_bridge:
                    live_bridge_check = ROOT / "tools" / "validation" / "check_mmo_step55_live_receiver_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step56b_progress_bridge:
                    progress_bridge_check = ROOT / "tools" / "validation" / "check_mmo_step56b_clean_db_progress_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step59_item_interactive_progress_bridge:
                    step59_check = ROOT / "tools" / "validation" / "check_mmo_step59_clean_db_item_interactive_progress_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step60_equipment_bridge:
                    step60_check = ROOT / "tools" / "validation" / "check_mmo_step60_clean_db_equipment_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step67_interactive_use_bridge:
                    step67_check = ROOT / "tools" / "validation" / "check_mmo_step67_interactive_use_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step68_drop_loot_bridge:
                    step68_check = ROOT / "tools" / "validation" / "check_mmo_step68_drop_loot_inventory_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step93_save_checkpoint_quest_utf8_bridge:
                    step93_check = ROOT / "tools" / "validation" / "check_mmo_step93_save_checkpoint_quest_utf8_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step94_server_save_checkpoint_manifest:
                    step94_check = ROOT / "tools" / "validation" / "check_mmo_step94_server_save_checkpoint_manifest.py"
                ...
                if manifest["status"] == "passed" and args.with_step95_save_slot_catalog_db_continue_bridge:
                    step95_check = ROOT / "tools" / "validation" / "check_mmo_step95_save_slot_catalog_db_continue_bridge.py"
                ...
                if manifest["status"] == "passed" and args.with_step96_db_save_checkpoint_snapshots:
                    step96_check = ROOT / "tools" / "validation" / "check_mmo_step96_db_save_checkpoint_snapshots.py"
                ...
                if manifest["status"] == "passed" and legacy_step97_restore_enabled:
                    step97_check = ROOT / "tools" / "validation" / "check_mmo_step97_db_save_checkpoint_restore_bridge.py"
                ...
                if manifest["status"] == "passed" and legacy_step98_strict_enabled:
                    step98_check = ROOT / "tools" / "validation" / "check_mmo_step98_strict_db_continue_restore.py"
