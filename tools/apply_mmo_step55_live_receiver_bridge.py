#!/usr/bin/env python3
"""Install Step55e minimal MySQL live receiver bridge.

This additive SQL surface is required when a clean MySQL DB is rebuilt from the
pre-Xardas SQLite capture and then used by run_mmo_action_receiver.py with
--enqueue-outbox. It creates the missing server session/outbox/worker tables and
procedures such as mmo_login_character and mmo_enqueue_server_action.
"""
...
DEFAULT_SQL = ROOT / "server" / "sql" / "step55_live_receiver_bridge.sql"

REQUIRED_TABLES = [
    "server_sessions",
    "character_checkpoint_audit",
    "mmo_server_action_outbox",
    "mmo_server_action_worker_runs",
    "mmo_server_action_worker_results",
]
REQUIRED_VIEWS = [
    "v_active_server_sessions",
    "v_character_latest_checkpoint",
    "v_server_action_outbox",
    "v_pending_server_actions",
]
REQUIRED_ROUTINES = [
    "mmo_login_character",
...
def missing(result: dict[str, Any]) -> dict[str, list[str]]:
    return {
        ...
        "views": [name for name, ok in result["views"].items() if not ok],
        "routines": [name for name, ok in result["routines"].items() if not ok],
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Install Step55e minimal MySQL live receiver bridge.")
