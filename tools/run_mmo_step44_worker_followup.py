#!/usr/bin/env python3
"""Run the Step44 post-capture MySQL follow-up pipeline.
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Step44 worker follow-up: optional pickup fixture, worker, gameplay domain checker.")
    ap.add_argument("--url", required=True)
    ap.add_argument("--session-key", required=True)
    ap.add_argument("--worker-id", default="dev-resolved-worker-step44")
    ...
    ap.add_argument("--summary-json", default="runtime/mmo_server_step43_summary.json")
    ap.add_argument("--output", default="runtime/mmo_step44_live_gameplay_domains.json")
    ap.add_argument("--manifest", default="runtime/mmo_step44_worker_followup_manifest.json")
    ...
    checker = script_dir / "check_mmo_step44_live_gameplay_domains.py"
    check_cmd = [
    ...
    results.append(run("step44_domain_check", check_cmd, allow_failure=True))
