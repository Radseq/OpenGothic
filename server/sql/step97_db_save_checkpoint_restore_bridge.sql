-- Step97: export latest DB-native save checkpoint as the bootstrap snapshot.
--
-- Step96 materialized normalized save checkpoint tables. This step makes those
-- tables useful for restore: the C++ UDP server can ask MySQL for the latest
-- checkpoint and send it as the normal mmo_bootstrap_snapshot_v1 JSON payload.
-- It remains additive and server-bound. Native .sav compatibility is unchanged.
...
CREATE TABLE IF NOT EXISTS mmo_save_checkpoint_world_clock_snapshot (
  ...
  CONSTRAINT mmo_save_clock_snapshot_manifest_fk FOREIGN KEY (manifest_id) REFERENCES mmo_save_checkpoint_manifests (manifest_id) ON DELETE CASCADE,
  CONSTRAINT mmo_save_clock_snapshot_json_ck CHECK (JSON_VALID(state_payload))
...
proc: BEGIN
  ...
  IF v_world_instance_id IS NULL THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='mmo_materialize_save_checkpoint_world_clock_snapshot_v1: invalid manifest';
  END IF;

  DELETE FROM mmo_save_checkpoint_world_clock_snapshot WHERE manifest_id = p_manifest_id;
...
DELIMITER ;;
CREATE FUNCTION mmo_build_latest_save_checkpoint_bootstrap_snapshot_v1(
...
BEGIN
  ...
  RETURN CONCAT(
    ...
    ',"script_state_truncated":', IF(v_script_int_count > 16384, 'true', 'false'),
...
SELECT
  ...
  CHAR_LENGTH(mmo_build_latest_save_checkpoint_bootstrap_snapshot_v1((
    SELECT ss.session_id
      FROM server_sessions ss
     WHERE ss.character_id = sm.character_id
       AND ss.world_instance_id = sm.world_instance_id
     ORDER BY ss.started_at DESC
     LIMIT 1
  ))) AS exported_bootstrap_bytes
FROM mmo_save_checkpoint_manifests sm
JOIN characters c ON c.character_id = sm.character_id;
