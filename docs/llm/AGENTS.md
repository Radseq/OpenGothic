...
intent producer and an authoritative-state projector, never a gameplay
authority.

## Required context

Read `README.md`, `current-state.md` and `next-work.md`. Read the relevant ADR
before changing a boundary. Use the repository index described by the root LLM
context to retrieve implementation and tests; do not replace retrieval with a
hand-maintained path catalog.

## Ownership rules

- Server gameplay truth belongs to `src/server`.
- Binary contracts and neutral identifiers belong to `src/shared`.
...
- Distinguish implemented, unit-tested, composition-integrated and
