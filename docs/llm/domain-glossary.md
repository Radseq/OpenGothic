...
Client mode enabled by `-mmo-client-server`. The client sends gameplay intents
to the C++ MMO server and applies server bootstrap materialization.
...
Stable key identifying the server's active game/mod content version. Clients
declare a manifest hash; the server validates it.

## `mmo_content_build`

Build-time MySQL schema containing parsed ZEN/DAT/OU content for a content
revision.

## Runtime Read-Model

Generated server-cache artifact exported from `mmo_content_build`. Current
schema: `mmo.content_build_runtime_read_model.v1`.

## `world_instance`

Future authoritative server simulation instance. It should load one content
read-model/cache and tick NPC/script/perception logic for all active players in
that instance.
